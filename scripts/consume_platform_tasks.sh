...
# Consume platform-dispatched tasks from Postgres (alert_event) for workspace_key='agent_builder'
...
mapfile -t rows < <(./scripts/psql_exec.sh -Atq -c "select id, payload::text from alert_event where id > ${last:-0} and workspace_key='agent_builder' and source='platform' and title='platform_task' order by id asc;")
...
if [ "$count" -gt 0 ]; then
  ack=$(printf '{"consumer_workspace_key":"%s","consumed":%s,"last_event_id":%s}' "agent_builder" "$count" "$max_id")
